    ...
    if (frappe.session.user === "Guest") {
        ...
        return frappe.call({
            ...
            callback: function (r) {
                ...
                let toastMsg = "";
                if (opts.qty === 0) {
                    toastMsg = __("Item removed from cart");
                } else {
                    const cartItems = r.message && r.message.items ? r.message.items : [];
                    const existingItem = false;

                    if (existingItem && existingItem.qty > 1) {
                        toastMsg = __("Cart updated");
                    } else {
                        toastMsg = __("Item added to cart");
                    }
                }

                // ✅ Show toast (uses your existing function)
