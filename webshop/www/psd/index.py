...
def get_context(context):
	...
	offers = frappe.get_all(
		"Website Offer",
		filters={"docstatus": 0},
		fields=["offer_title", "offer_subtitle", "offer_details", "parent"],
		limit_page_length=1
	)
	item_route_map = {item["name"]: item["route"] for item in products}

	for offer in offers:
		offer["route"] = item_route_map.get(offer["parent"], "")

	context.products = products
	context.item_groups = frappe.get_all(
		"Item Group",
		filters={"parent_item_group": "Custom"},
