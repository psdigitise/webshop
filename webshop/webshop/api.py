...
@frappe.whitelist()
def get_website_items(search=None, page=1, page_size=12):
    filters = {}
    if search:
        filters["web_item_name"] = ["like", f"%{search}%"]

    items = frappe.get_all(
        "Website Item",
        fields = ["web_item_name", "item_code", "item_name", "route", "item_group","stock_uom", "brand", "published", "has_variants"],
        filters=filters,
