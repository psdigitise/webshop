...
class ItemReview(Document):
	...
	def validate(self):
		update_average_ratings(self.website_item)

def update_average_ratings(website_item):
    avg_raw = frappe.db.sql("""
        SELECT AVG(rating) FROM `tabItem Review`
        WHERE website_item = %s
    """, website_item)[0][0] or 0

    avg_decimal = Decimal(str(avg_raw)) / Decimal('0.2')
    avg_stars = avg_decimal.quantize(Decimal('1'), rounding=ROUND_HALF_UP)

    frappe.db.set_value('Website Item', website_item, 'average_rating', float(avg_stars))
